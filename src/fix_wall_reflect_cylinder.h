...
#ifdef FIX_CLASS
// clang-format off
FixStyle(wall/reflectcylinder,FixWallReflectCylinder);
// clang-format on
#else

#ifndef LMP_FIX_WALL_REFLECTCYLINDER_H
#define LMP_FIX_WALL_REFLECTCYLINDER_H

#include "fix.h"

namespace LAMMPS_NS {

class FixWallReflectCylinder : public Fix {
 public:
  enum { XLO = 0, XHI = 1, YLO = 2, YHI = 3, ZLO = 4, ZHI = 5 };
  enum { NONE = 0, EDGE, CONSTANT, VARIABLE };

  FixWallReflectCylinder(class LAMMPS *, int, char **);
  virtual ~FixWallReflectCylinder();
  int setmask();
  void init();
  void post_integrate();
 ...
 protected:
  ...
  virtual void wall_particle(double R, double yc, double zc);
};
